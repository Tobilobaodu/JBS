...
### Extension #1 — ATS structural validation — ✅ implemented and live-verified (2026-08-11)
...
> ✅ **Live verification (2026-08-11).** Migration `007`'s upgrade/downgrade round-trip (`001`→`007`, `-1`, back to head) confirmed clean against an isolated Postgres instance (`cv_tailoring_verify`, dropped afterward). All 32 ATS-related tests pass against real Postgres; broader suite 138/139 at the time (1 pre-existing, unrelated failure, confirmed via empty `git diff` on that test's own code). This line previously said "not yet live-verified" after the verification had already been done in an earlier pass — the doc simply hadn't been corrected; fixed now.
...
### Done (for reference — full write-ups linked above, not repeated here)

1. ~~Commit Sprint 1 and Sprint 2~~ — `53f3452`, `25ad817` on local `main`.
2. ~~Push local `main` to `origin`~~ — confirmed in sync, 0 ahead/0 behind, still true as of this update.
3. ~~Pin the LLM provider/model decision~~ — OpenAI API, `gpt-4o-mini`, Data Controls confirmed off.
4. ~~Wire up a Celery beat scheduler~~ — schedule + `beat` service + `worker_maintenance` all added. (Not yet live-verified end-to-end — carried forward as item 3 below.)
5. ~~Investigate the `cv_tailoring` dev database's stuck `alembic_version`~~ — resolved via `alembic stamp 005`.
6. ~~Sprint 3 — tailored CV generation~~ — built and live-verified, re-confirmed in a second pass today (2026-08-11) after a stale test run briefly looked like a regression and wasn't one.
7. ~~Extension #1 — ATS structural validation~~ — built and live-verified; the doc's stale "not yet live-verified" line is fixed as of this update.

### Open, in priority order

1. **Full end-to-end walkthrough through the real deployed stack.** `docker compose up` (api, all Celery workers including `worker_cv_generate`, Postgres, Redis) with a real `OPENAI_API_KEY`, generating an actual draft through the running API instead of a fake LLM client. Every layer up to the OpenAI call itself has been verified; the real call has never been made.
2. **Sprint 4 — real cover letter generation.** Now unblocked; reuse Sprint 3's `llm_client.py` + evidence-binding/verification infrastructure directly rather than building a second pipeline. See the Sprint 4 plan below.
3. **Live-verify the Celery beat scheduler end-to-end.** `docker compose up` with `beat` + `worker_maintenance` running, confirm `cleanup_expired_trial_sessions` actually fires on schedule and executes — only ever config/import-checked so far, never watched run.
4. **Fix `schemas/cover_letter.py`'s field-alias bug.** Inverted convention (camelCase field name, snake_case alias) silently produces snake_case JSON instead of the camelCase `05-openapi.yaml` documents, for every field where the two differ (`currentStep`, `questionSetVersion`, `createdAt`, etc.). Found while building Sprint 3, confirmed directly against FastAPI's actual serialization behavior, never fixed — small, mechanical, same fix pattern already applied correctly in `schemas/tailored_cv.py`.
5. **Fix the 6 case-sensitivity assertion bugs in `test_ssrf_fetch.py`.** `pytest.raises(SSRFRejection, match="scheme")` against an actual message of `"Scheme '...' is not allowed..."` — `match` is case-sensitive, so these 6 parametrized cases fail regardless of whether the underlying rejection logic is correct. Low risk (SSRF protection itself works — this is a test-file typo), but as written these assertions can never catch a real regression in scheme-rejection behavior, which defeats their purpose.
6. **Decide what to do with `test_pinned_dns_does_not_affect_other_hostnames`.** Depends on resolving a reserved `.example` domain via real DNS, which doesn't work in this environment (or in any environment without a catch-all resolver) — needs a mock for the "other hostname" case instead of a real `socket.getaddrinfo` call, or an explicit skip-if-no-network marker.
7. **Decide whether to extend Phase 2 extraction to populate education/certifications/projects.** Currently every tailored CV draft ships with these sections empty — correct non-fabrication behavior given `CvEducationItem` is never populated anywhere and no certification/project tables exist, but it's a real product gap, not just a technicality. Needs a product call on whether it's in scope before Sprint 5, or later.
8. **Sprint 5 — exports and remaining Phase 5 surface.** `POST /exports/*` (none exist yet), multi-job-post coverage reporting (Extension #2). See the Sprint 5 plan below.
9. **Sprint 6 — full hardening pass and pre-launch review.** Full `09-test-plan.md`/`10-security-plan.md` pass, incident-response tabletop, load/soak test, external security review. See the Sprint 6 plan below.

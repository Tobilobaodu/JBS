...
Base commit for everything in this document: `64cfbbd` (HEAD as of the last review), **plus** the Sprint 1 changes (2026-08-10) and the Sprint 2 changes (2026-08-11) described below — both confirmed still sitting as uncommitted working-tree changes, not new commits. Don't treat either sprint as merged until it's actually committed.
...
| — Anonymous trial support | ✅ Complete, verified live | Sprint 2 (2026-08-11): schema, dependency, route wiring, claim-on-registration, abuse controls, and expiry cleanup all written and verified against a real Postgres instance — migration round-trip, CHECK constraints, concurrency race, claim-transaction atomicity, IDOR, and the cleanup task's full delete order all confirmed. Two pre-existing ownership gaps found and fixed along the way. Not yet committed; no scheduler exists yet to invoke the cleanup task periodically. |
...
|---|---|---|---|
| LLM provider and model | Phase 3 (Sprint 3) | Unresolved | `OPENAI_API_KEY`/`OPENAI_MODEL` exist as config fields with `gpt-4o` as a placeholder default — scaffolding for the *shape* of the config, not a pinned decision. **Recommend starting this now, in parallel with Sprint 2**, not waiting for Sprint 3 — this product sends real people's CVs to a third-party model, which can mean a DPA, vendor security review, or procurement sign-off with lead time unrelated to engineering velocity. Discovering that mid-Sprint-3 would stall the sprint that's supposed to be the actual product. |
...
All four items done and tested (47 passed, 1 pre-existing unrelated skip); see Phase 3 and Phase 5 sections above for what changed. **Not yet committed** — sitting as working-tree changes pending review.
...
1. **Commit Sprint 1 and Sprint 2** — both are verified and ready; see the "Anonymous trial support" section above for the full verification write-up.
2. **Start the LLM provider/model decision now**, in parallel with Sprint 3 kickoff — see Open Decisions above; this has non-engineering lead time (DPA/procurement) that can otherwise stall Sprint 3 later.
3. **Wire up a Celery beat (or equivalent) scheduler** for `cleanup_expired_trial_sessions` — written and tested, but nothing invokes it periodically yet. Small, but genuinely open.
4. **Sprint 3 immediately follows** — track Sprint 2 + Sprint 3 combined duration as the number that determines whether this plan hits its dates, not either sprint's estimate alone. Sprint 2's actual velocity (implemented, IDOR-audited, and DB-verified in one session) is worth using to sanity-check the Sprint 3 estimate rather than the original Task-2.3-based planning number.
